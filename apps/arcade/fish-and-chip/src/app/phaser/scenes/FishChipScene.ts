...
export class FishChipScene extends Scene {
    ...
    // All assigned in create(), which Phaser runs before anything below can be
    // reached, so they are asserted rather than optional.
    ...
    createSandStorm() {
         ...
         */
        const rectanglePool: Phaser.GameObjects.Rectangle[] = [];
         ...
         */
        const getRectangle = (): Phaser.GameObjects.Rectangle =>
            rectanglePool.pop() ??
            this.add.rectangle(0, 0, this.cameras.main.width, 20, 0xc2b280, 0.2);
