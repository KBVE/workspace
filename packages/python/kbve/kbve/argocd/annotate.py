...
def main() -> int:
    ...
    if not args.apps_dir.is_dir():
        print(f"kbve-argocd-annotate: no directory at {args.apps_dir}",
              file=sys.stderr)
        return 2
    ...
    # Exit status is what makes validate and drift usable as a gate. They used
    # to print a wall of red crosses and return None, so a pipeline that ran
    # either one passed no matter what it found.
    if args.mode in ("validate", "drift"):
        return 1 if (result["invalid"] or result["missing"]) else 0
    return 0
