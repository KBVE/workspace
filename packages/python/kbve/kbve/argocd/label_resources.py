...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="kbve-argocd-label",
        description="Apply kbve.com labels to Kubernetes resources based on ArgoCD annotations",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--mode",
        # validate and report were listed here and implemented as `pass` with a
        # TODO: they printed a header, did nothing, and exited 0, so asking for
        # either looked exactly like a clean run. Offering only what exists
        # makes the request an argparse error instead of a false pass.
        choices=["label", "drift"],
        required=True,
        help="Operation mode",
    )
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="directory of ArgoCD Application YAML to walk",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not args.apps_dir.is_dir():
        print(f"kbve-argocd-label: no directory at {args.apps_dir}",
              file=sys.stderr)
        return 2
    ...
    # drift changes nothing and reports what label would change; anything it
    # would have touched is drift, and drift is what a gate wants to fail on.
    if args.mode == "drift":
        return 1 if result["updated_resources"] else 0
    return 0
