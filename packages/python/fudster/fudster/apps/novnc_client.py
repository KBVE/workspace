...
class NoVNCClient:
    ...
    def start_websockify_server(self, target_host: str, target_port: int):
        # get_running_loop, not get_event_loop: this hands work to the loop's
        # executor, so it only means anything when a loop is already running.
        # get_event_loop used to conjure one when called from a thread that had
        # none -- 3.14 raises instead, and the work would have gone to a loop
        # nothing was driving anyway.
